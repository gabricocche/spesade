...
export default function WalkCountSheet({ selectedItem, sheetValue, setSheetValue, onClose, onSave }: WalkCountSheetProps) {
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 bg-card border-t border-border/40 rounded-t-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-full duration-300 touch-none">
        ...
        <div className="flex justify-between items-start mt-2 mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Quantità bersaglio: {selectedItem.target_quantity} {selectedItem.unit}</p>
            <p className="text-xs text-muted-foreground/70 mt-0.5">Indica la quantità corrente</p>
          </div>
        ...
        <button onClick={onSave} className="w-full bg-primary text-primary-foreground font-bold py-4 rounded-xl text-lg flex items-center justify-center gap-2">
          <Save className="w-5 h-5" /> Conferma conteggio
        </button>
      </div>
    </>
  );

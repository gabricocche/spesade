...
from backend.src.schemas.lists import AutoGenerateRequest, InventoryCheck, ListCreate, ListItemCreate, ListStatus
...
class CrudTestCase(unittest.TestCase):
    ...
    def test_toggle_item_bought(self):
        ...
        # test in bozza (deve riuscire)
        result = lists.toggle_item_bought(self.db, lista.id, item.id)
        self.assertTrue(result["bought"])

        # test in pending (deve riuscire)
        lists.update_list_status(self.db, lista.id, "pending")
        result = lists.toggle_item_bought(self.db, lista.id, item.id)
        self.assertFalse(result["bought"])

        # test in completata (deve fallire)
        lists.update_list_status(self.db, lista.id, "completed")
        with self.assertRaises(HTTPException) as ctx:
            lists.toggle_item_bought(self.db, lista.id, item.id)
        self.assertEqual(ctx.exception.status_code, 400)

    def test_state_machine_transitions(self):
        # verifica che solo le transizioni permesse vadano a buon fine
        lista = lists.create_list(self.db, ListCreate(name="Lista state machine"))

        # valida: bozza → in attesa
        result = lists.update_list_status(self.db, lista.id, "pending")
        self.assertEqual(result["new_status"], "pending")

        # valida: in attesa → completata
        result = lists.update_list_status(self.db, lista.id, "completed")
        self.assertEqual(result["new_status"], "completed")

        # non valida: completata → bozza (stato terminale)
        with self.assertRaises(HTTPException) as ctx:
            lists.update_list_status(self.db, lista.id, "draft")
        self.assertEqual(ctx.exception.status_code, 400)

        # non valida: bozza → completata (salta uno stato)
        lista2 = lists.create_list(self.db, ListCreate(name="Lista salto stato"))
        with self.assertRaises(HTTPException) as ctx:
            lists.update_list_status(self.db, lista2.id, "completed")
        self.assertEqual(ctx.exception.status_code, 400)

        # stesso stato → idempotente, nessun errore
        result = lists.update_list_status(self.db, lista2.id, "draft")
        self.assertEqual(result["new_status"], "draft")

    def test_delete_list_status_guard(self):
        # una lista in pending o completata non può essere eliminata
        category = categories.create_category(self.db, CategoryCreate(name="Test Cat"))
        lista = lists.create_list(self.db, ListCreate(name="Lista da non eliminare"))

        # portiamo la lista in pending
        lists.update_list_status(self.db, lista.id, "pending")

        # eliminare una lista in pending deve fallire con 400
        with self.assertRaises(HTTPException) as ctx:
            lists.delete_list(self.db, lista.id)
        self.assertEqual(ctx.exception.status_code, 400)

        # portiamo in cancelled → l'eliminazione deve riuscire
        lists.update_list_status(self.db, lista.id, "cancelled")
        result = lists.delete_list(self.db, lista.id)
        self.assertIn("message", result)
